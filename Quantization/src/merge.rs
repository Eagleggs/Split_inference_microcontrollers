...
pub fn merge_batchnorm(layers:HashMap<i32,Box<dyn Layer>>){
    ...
    for i in 1..=layers.len(){
        ...
        if layer.identify() == "Convolution" {
            let next_layer = layers.get(&(i as i32 + 1)).unwrap();
            let Convolution(info) = layer.get_info() else {panic!("impossible to decode convolution info from none convolution layer")};
            ...
            if next_layer.identify() == "Batchnorm2d" {
                ...
                modified_mapping.insert(i as i32,LayerWrapper::Convolution(fused_conv));
            }
        }
        else if layer.identify() == "Relu6" {
            let input_shape = layer.get_output_shape();
            let relu6 = Relu6{ input_shape };
            modified_mapping.insert(i as i32, LayerWrapper::ReLU6(relu6));
        }
    }
