...
pub fn calculate_quantization(original_weights: Vec<Vec<WeightUnit>>,original_mapping:Vec<Mapping>,weight_scales : Vec<f32>,weight_zero_points : Vec<u8>,layer_id:usize)->(Vec<Vec<QuantizedWeightUnit>>,Vec<QuantizedMapping>) {
    ...
    let num_workers = original_mapping.len();
    let mut quant_weights: Vec<Vec<QuantizedWeightUnit>> = Vec::new();
    let mut quant_mapping: Vec<QuantizedMapping> = Vec::new();
    let mut m = 0.;
