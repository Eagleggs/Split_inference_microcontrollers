...
void distributed_computation(std::vector<Weight>& w, byte* input_distribution,byte* result,byte* overflow,int len) {
  ...
  if (w[0].i.type == Type::Convolution) { 
    ...
    for(int i = 0; i < w.size(); i++){
      ...
      // Serial.println("start calculation2");
      while(w[i].count > 0){
        ...
        for(int c = 0; c < info.i_pg; c++){
          ...
          for(int j = 0; j < info.k[0]; j++){
            ...
            for(int k = 0; k < info.k[1]; k++){
              ...
              // }
              acc += (input_distribution[index] - w[i].zero_points[0]) * (w[i].data[(c * info.k[0] * info.k[1] + j * info.k[1] + k)] - w[i].zero_points[1]);
            }
          }
        }
        acc += w[i].bias;
        acc = int(round(float(acc) * w[i].m));
        acc += w[i].zero_points[2];
        if(acc < 0) acc = 0;
        if(acc > 255) acc = 255; 
